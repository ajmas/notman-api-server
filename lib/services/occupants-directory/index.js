...
var alumniPath = config.cacheDir + '/alumni.csv';
var peoplePath = config.cacheDir + '/people.csv';
...
function loadAlumni(alumniCsv) {
    return new Promise(function(resolve, reject) {
        var stream = fs.createReadStream(alumniCsv);

        alumni = [];

        var csvStream = fastcsv
            .fromStream(stream, { delimiter: ',', headers : true, strictColumnHandling: false })
            .on("data", function(data) {
                alumni.push(data);
            })
            .on("end", function() {
//                 alumni = populatLogoPaths(alumni);
                resolve(alumni);
            })
            .on("error", function(error,data) {
                reject(error);
...
function handleGetAlumni(req, res, next) {
    res.json(alumni.map(function (entry) {
        return {
           organisation: entry.organisation,
           website: (entry.website && entry.website.trim().length > 0)?entry.website:undefined,
           type: entry.type
        }
    }));
}
...
function getWebsite(organisation) {
    let entry;
    for (let i=0; i<alumni.length; i++) {
        console.log(organisation.toLowerCase(), alumni);
        if (organisation.toLowerCase() === alumni[i].organisation.toLowerCase()) {
            entry = alumni[i];
            break;
        }
    }
...
function handleGetOccupant (req, res, next) {
    try {
        const occupantName = req.params[0].trim();
        let occupant;

        if (occupantName && occupantName.length > 0) {
            var filteredOccupants = occupants.filter(function(entry) {
                return (entry.occupant.toLowerCase() === occupantName.toLowerCase());
            });

            if (filteredOccupants.length > 0) {
                occupant = filteredOccupants[0];
            }
        }

        // TODO Not a great solution, but temporary work around for an issue in the data
        if (occupant && !occupant.avatar) {
            occupant = undefined;
        }

        if (!occupant) {
            res.status(404).send("No corresponding entry could be found");
            return;
        }

        const name = occupant.occupant  ;
        const avatar = occupant.avatar;
        const website = getWebsite(occupant.occupant);
              ...
              "@graph": [
                {
                  ...
                  "schema:name": name,
//                   "schema:url": "https://sniffypedia.org/Organization/Apple_Inc/",
                  "schema:logo": "http://notman.herokuapp.com/api/directory/" + escape(name) + "/avatar",
                  ...
                  "location": {
                     ...
                     "address": {
                       ...
                       "addressRegion": "QC",
                       "postalCode": "H2X 1X2",
                       "streetAddress": "51 Sherbrooke Ouest"
                    }
                 }
              }]
            }
...
module.exports.handleGetOccupant = handleGetOccupant
